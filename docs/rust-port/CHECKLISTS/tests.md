...
- [x] **CV-01** | Conversion pipeline: golden 3-page conversion (8 ordered steps, verify gate, both-columns cover update, page recount, original archived), verify gate both ways (sequential names, ComicInfo-not-first, empty title, valid archive), count guard → pdfium lossy fallback (F1 resolved option A: 3 ordered JPEGs, `method: "pdfium"`, `lossless: false`, source untouched, `Count mismatch` logged) + PDF thumbnail via page-1 render (scanner-scheme file, fit-inside 600×800 q82), rasteriser-absent loud failure in its own target (process-global first-call-wins binding), keep_original=false deletes lossless source, archive collision uniquifies -1, stale queue rows complete cleanly, cancel leaves pending rows, metadata job resumable (D-metadata-queue: marker move leading-prefix-only, errors cap), originals walk/restore/purge incl. _lossy strip + never-overwrite | semantic | synthetic image-PDF fixtures | `cargo test --test conversion` + `cargo test --test raster_absent`
- [x] **SY-01** | Batch sync: retry ladder (500→500→200 success, backoff 2000+n*1000), 429+Retry-After waits (preserved quirk: the for-loop's continue DOES consume an attempt — the source comment claims otherwise; code is the contract), 3 failures → queue error, per-item guard message, 90%-of-endpoint-limit pacing with work-counts-against-interval (anon 3334ms / keyed 1500ms), cooperative cancel leaves pending, resume via empty ids (total = requested size), main-side commit: library row + full gallery-cache enrichment (japanese title etc.) + page count re-derived, series preserved from our DB | semantic | scripted transport | `cargo test --test sync_differential`
- [x] **KV-01** | Replay shapes: /api/Account in-band ok + never-throws, libraries enumeration, scan-folder POST with translated path + body apiKey, search queryString, delete-multiple dedup/filter, getItemCount 60s cache; LIVE (id 6): connection ok, both ids enumerate, exact-name skip-only, wrong-key/unreachable negatives | invariant | Doujin-Test (id 6) | `cargo test --test kavita_acceptance` + `-- --ignored`
- [x] **KV-02** | Production-library protection: id 5 refused by helper on every mutating path; server chapterCount pinned before/after every live test (280+ files server-wide; any id-5 mutation would move it); all mutations in id 6 only | invariant | server | embedded in every `kavita_acceptance` live test
- [x] **CR-01** | Boot after a mid-batch kill: scan_queue wiped, download downloading/converting requeued + pages/scratch purged, conversion converting→pending (history kept), sync syncing→pending, orphans swept; second boot is a no-op; each engine's recovered rows claimable; a second crash+boot also recovers | invariant | scratch DB + dirs | `cargo test --test crash_recovery -- --test-threads=1`
