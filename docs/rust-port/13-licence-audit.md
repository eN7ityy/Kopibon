...
| `pdfium-render` 0.9.3 (06) | **MIT OR Apache-2.0 (verified** against the crate `Cargo.toml` at adoption; `Cargo.lock` pin `826f8f64…`) | Notice preservation | Generated notices file. Default features (`pdfium_latest`, `image_025`, `thread_safe`); binds the vendored pdfium binary at runtime (see next row), never a shelled-out tool |
| vendored `libpdfium.so` (06, `third-party/pdfium/`) | **BSD-3-Clause (verified** — `LICENSE.pdfium` in-tree, pdfium-binaries `chromium/8035`, pdfium 154.0.8035.0, non-V8 linux-x64, 7.7 MB unpacked) + bundled third-party notices (zlib, lcms, freetype, libpng, libjpeg-turbo, icu, abseil, … — all permissive) | BSD notice + disclaimer reproduced in the generated notices file | Runtime `bind_to_library` beside the binary (Tauri `resources` at packaging time); dev/test fallback to the in-tree copy is `#[cfg(debug_assertions)]` only, never in release builds |
| `reqwest` with `rustls-tls` (04, 07) | MIT OR Apache-2.0 | Notice preservation | **Superseded at B2-auth: `ureq` chosen instead** (blocking API fits the sync `Transport` trait; no async runtime in the shell). Row kept for the record |
| `ureq` 3.4.0 (08, B2-auth) | **MIT OR Apache-2.0 (verified** against the crate `Cargo.toml` at adoption) | Notice preservation | Generated notices file. Blocking HTTPS transport for `auth:validateKey`/`restoreAuthFromDb` (`http_status_as_error(false)` so the core's 429/error mapping sees raw statuses); pulls `rustls` + `ring` (next row) |
| `rustls` 0.23.43 + `ring` 0.17.14 via ureq (08, B2-auth) | **Apache-2.0 OR ISC OR MIT (rustls, verified); Apache-2.0 AND ISC (ring, verified** — both against the crate `Cargo.toml` files at adoption) | Notice preservation (+ `ring`'s OpenSSL-derived attribution per §2-row above — verify the exact notices text at generation time) | Generated notices file; TLS backend choice thus recorded: rustls/ring, no aws-lc |
| `keyring` 4.2.0 (08, B2-auth) | **MIT OR Apache-2.0 (verified** against the crate `Cargo.toml` at adoption) | Notice preservation | Generated notices file. `safeStorage` replacement (02-ipc-surface §6): OS keychain via Secret Service on Linux (`zbus` MIT + `zbus-secret-service-keyring-store` MIT OR Apache-2.0, both transitive); plaintext passthrough when unavailable |
